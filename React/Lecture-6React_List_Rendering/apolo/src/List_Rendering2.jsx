...
const List_Rendering2 = () => {

  // const ProductList = ProductData.map((list) => <li className='text-center'>{list}</li>)

  // const ProductList = ProductData.filter((data) => data.category === 'laptops')

  const ProductList = ProductData.filter((data) => data.price > 500 && data.price < 1000 || data.price > 1400)

  return (
    <div>
       <h1 className='text-4xl text-center font-black bg-blue-500 p-2'>React List Rendering</h1>
        <div className='flex flex-wrap justify-between mt-16 p-4'>
          {
            ProductList.map((item) => {
              return(
                <>
                  <div className='h-78 w-72 border-2 border-solid border-black mb-4 p-4'>
                    <div>
                      <img className='h-28 w-28' src={item.images} alt="" />
                    </div>
                    <div>
...
export default List_Rendering2
